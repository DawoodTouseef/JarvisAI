...
class RAGRouter(RAGAgent):
    ...
    async def _async_route(self, query: str) -> Tuple[RAGAgent, int]:
        description_str = "\n".join(
            [f"[{i + 1}]: {description}" for i, description in enumerate(self.agent_descriptions)]
        )
        prompt = RAG_ROUTER_PROMPT.format(query=query, description_str=description_str)
    ...
    async def async_retrieve(self, query: str, **kwargs) -> Tuple[List[RetrievalResult], int, dict]:
        agent, n_token_router = await self._async_route(query)
        retrieved_results, n_token_retrieval, metadata = await agent.async_retrieve(query, **kwargs)
        return retrieved_results, n_token_router + n_token_retrieval, metadata
    ...
    async def async_query(self, query: str, **kwargs) -> Tuple[str, List[RetrievalResult], int]:
        agent, n_token_router = await self._async_route(query)
        answer, retrieved_results, n_token_retrieval = await agent.async_query(query, **kwargs)
        return answer, retrieved_results, n_token_router + n_token_retrieval
