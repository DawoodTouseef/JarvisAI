...
class ChainOfRAG(RAGAgent):
    ...
    async def _async_reflect_get_subquery(self, query: str, intermediate_context: List[str]) -> Tuple[str, int]:
        chat_response = self.llm.chat(
            [
                {
                    "role": "user",
    ...
    async def _async_retrieve_and_answer(self, query: str) -> Tuple[str, List[RetrievalResult], int]:
        consume_tokens = 0
        if self.route_collection:
            selected_collections, n_token_route = self.collection_router.invoke(
                query=query, dim=self.embedding_model.dimension
    ...
    async def _async_get_supported_docs(
        self,
        retrieved_results: List[RetrievalResult],
        query: str,
        intermediate_answer: str,
    ...
    async def _async_check_has_enough_info(
        self, query: str, intermediate_contexts: List[str]
    ) -> Tuple[bool, int]:
        if not intermediate_contexts:
            return False, 0
    ...
    async def async_retrieve(self, query: str, **kwargs) -> Tuple[List[RetrievalResult], int, dict]:
        """
        Async retrieves relevant documents based on the input query and iteratively refines the search.
        """
        max_iter = kwargs.pop("max_iter", self.max_iter)
        intermediate_contexts = []
        all_retrieved_results = []
        ...
        for iter in range(max_iter):
            log.color_print(f">> Iteration: {iter + 1}\n")
            followup_query, n_token0 = await self._async_reflect_get_subquery(query, intermediate_contexts)
            intermediate_answer, retrieved_results, n_token1 = await self._async_retrieve_and_answer(
                followup_query
            )
            supported_retrieved_results, n_token2 = await self._async_get_supported_docs(
                retrieved_results, followup_query, intermediate_answer
            )
            ...
            if self.early_stopping:
                has_enough_info, n_token_check = await self._async_check_has_enough_info(
                    query, intermediate_contexts
                )
                token_usage += n_token_check
    ...
    async def async_query(self, query: str, **kwargs) -> Tuple[str, List[RetrievalResult], int]:
        """
        Async query the agent and return the final answer.
        """
        all_retrieved_results, n_token_retrieval, additional_info = await self.async_retrieve(query, **kwargs)
        intermediate_context = additional_info["intermediate_context"]
        log.color_print(
            f"<think> Summarize answer from all {len(all_retrieved_results)} retrieved chunks... </think>\n"
        )
