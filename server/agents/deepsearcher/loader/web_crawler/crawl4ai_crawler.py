...
class Crawl4AICrawler(BaseCrawler):
    ...
    async def async_crawl_url(self, url: str) -> List[Document]:
        """
        Asynchronously crawl a single URL.
        """
        try:
            document = await self._async_crawl(url)
            return [document]
    ...
    def crawl_url(self, url: str) -> List[Document]:
        ...
        if loop and loop.is_running():
            import nest_asyncio
            nest_asyncio.apply()
            return asyncio.run(self.async_crawl_url(url)) # Fallback or use run_coroutine_threadsafe
        else:
            return asyncio.run(self.async_crawl_url(url))
    ...
    async def async_crawl_many_docs(self, urls: List[str]) -> List[Document]:
        """
        Asynchronously crawl multiple URLs.
        """
        try:
            return await self._async_crawl_many(urls)
        except Exception as e:
    ...
    def crawl_urls(self, urls: List[str], **crawl_kwargs) -> List[Document]:
        ...
        if loop and loop.is_running():
            # If we are in an async loop, we should ideally await, 
            # but since this is a synchronous interface, we might need nest_asyncio or similar.
            # However, in our agents, we are now calling the private _async methods directly.
            import nest_asyncio
            nest_asyncio.apply()
            return asyncio.run(self.async_crawl_many_docs(urls))
        else:
            return asyncio.run(self.async_crawl_many_docs(urls))
