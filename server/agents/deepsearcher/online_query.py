...
# from ..configuration import vector_db, embedding_model, llm
from . import configuration
from .vector_db.base import RetrievalResult
...
async def async_query(original_query: str, max_iter: int = 3) -> Tuple[str, List[RetrievalResult], int]:
    """
    Async query the knowledge base with a question and get an answer.
    """
    default_searcher = configuration.default_searcher
    return await default_searcher.async_query(original_query, max_iter=max_iter)
...
async def async_retrieve(
    ...
    """
    Async retrieve relevant information from the knowledge base without generating an answer.
    """
    default_searcher = configuration.default_searcher
    retrieved_results, consume_tokens, metadata = await default_searcher.async_retrieve(
        original_query, max_iter=max_iter
    )
    return retrieved_results, [], consume_tokens
