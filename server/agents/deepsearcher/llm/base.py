...
class BaseLLM(ABC):
    ...
    def literal_eval(response_content: str):
        ...
        code_block_match = re.search(r"```(?:python|json|str)?\n?(.*?)```", response_content, re.DOTALL)
        if code_block_match:
            try:
        ...
        # 2. Try direct literal_eval of the whole content (if it's a clean literal)
        try:
            return ast.literal_eval(response_content)
        except (ValueError, SyntaxError):
            pass

        # 3. Search for the first block that looks like a list or dict
        matches = re.findall(r"(\[.*?\]|\{.*?\})", response_content, re.DOTALL)
        for match in matches:
            try:
                return ast.literal_eval(match.strip())
            except (ValueError, SyntaxError):
                continue
